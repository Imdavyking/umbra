 ...
 */
export const getZKBadgeLedgerState = (
...
const resolve = async (
  ...
  rli: Interface,
  logger: Logger
): Promise<CertVerificationAPI | null> => {
  let api: CertVerificationAPI | null = null;
...
const displayLedgerState = async (
  providers: CertVerificationContractProviders,
  deployedStateraContract: DeployedCertVerificationOnchainContract,
  logger: Logger
): Promise<void> => {
  const contractAddress =
    deployedStateraContract.deployTxData.public.contractAddress;
  const ledgerState = await getZKBadgeLedgerState(providers, contractAddress);
  if (ledgerState === null) {
    logger.info(
    ...
    logger.info(
      `Current registered hashes is: ${ledgerState.registered_hashes.size()}`
    );
    logger.info(`Current verified users: ${ledgerState.verified_users}`);
  }
};

const displayDerivedLedgerState = async (
  currentState: DerivedCertVerificationContractState,
  logger: Logger
): Promise<void> => {
  console.log(`Current registered hashes is:`, currentState.registeredHashes);
  console.log(`Current verified users:`, currentState.verifiedUsers);
  console.log(
    `Current number of registered hashes is:`,
    currentState.registeredHashes.length
  );
};

const getUserPrivateState = async (
  providers: CertVerificationContractProviders
): Promise<ZkBadgePrivateState | null> =>
  providers.privateStateProvider
    .get(CertVerificationPrivateStateId)
...
const displayUserPrivateState = async (
  ...
  if (privateState === null)
    logger.info(
      `There is no private state stored at ${CertVerificationPrivateStateId}`
    );
  logger.info(`Current certificate is: ${privateState?.certificate}`);
};
``;
...
const circuit_main_loop = async (
  ...
  const CertVerificationAPI = await resolve(providers, rli, logger);
  if (CertVerificationAPI === null) return;

  let currentState: DerivedCertVerificationContractState | undefined;
  const stateObserver = {
    next: (state: DerivedCertVerificationContractState) => {
      currentState = state;
    },
  };

  const subscription = CertVerificationAPI.state.subscribe(stateObserver);
      ...
      switch (choice) {
        case "1": {
          await displayLedgerState(
            providers,
            CertVerificationAPI.allReadyDeployedContract,
            logger
          );
          break;
        }
        ...
        case "5": {
          await CertVerificationAPI.register(
            {
              bytes: utils.hexToUint8Array(
                await rli.question("Enter issuer public key (in hex): ")
              ),
            },
            BigInt(await rli.question("Enter issued at timestamp (in ms): ")),
            BigInt(await rli.question("Enter valid until timestamp (in ms): ")),
            Boolean(
              await rli.question("Is the certificate valid? (true/false): ")
            ),
            BigInt(await rli.question("Enter year of birth (4-digit): "))
          );
        ...
        case "6": {
          await CertVerificationAPI.verifyCertificates([]);
        ...
        case "7": {
          await CertVerificationAPI.checkVerification({
            issuer: {
              bytes: utils.hexToUint8Array(
                await rli.question("Enter issuer public key (in hex): ")
              ),
            ...
            issued_at: BigInt(
              await rli.question("Enter issued at timestamp (in ms): ")
            ),
            valid_until: BigInt(
              await rli.question("Enter valid until timestamp (in ms): ")
            ),
            is_valid: Boolean(
              await rli.question("Is the certificate valid? (true/false): ")
            ),
            owner: {
              bytes: utils.hexToUint8Array(
                await rli.question("Enter owner address(in hex): ")
              ),
            },
            year_of_birth: BigInt(
              await rli.question("Enter year of birth (4-digit): ")
            ),
          });
...
const displayComprehensiveWalletState = async (
  wallet: Wallet,
  currentContractState: DerivedCertVerificationContractState | undefined,
  logger: Logger
): Promise<void> => {
  const state = await Rx.firstValueFrom(wallet.state());
...
const buildWallet = async (
  ...
  if (seed !== undefined) {
    logger.info(`Using seed from environment variable: ${seed}`);
    return await buildWalletAndWaitForFunds(config, seed, "", logger);
  }
  while (true) {
...
export const run = async (
  ...
  if (dockerEnv !== undefined) {
    ...
    if (config instanceof StandaloneConfig) {
      config.indexer = mapContainerPort(env, config.indexer, "zkbadge-indexer");
      config.indexerWS = mapContainerPort(
        env,
        config.indexerWS,
        "zkbadge-indexer"
      );
      config.node = mapContainerPort(env, config.node, "zkbadge-node");
      config.proofServer = mapContainerPort(
        env,
        config.proofServer,
        "zkbadge-proof-server"
      );
    }
  }
