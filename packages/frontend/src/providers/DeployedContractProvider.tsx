...
export interface DeploymentProvider {
  ...
  readonly zkBadgeApi: DeployedCertVerificationAPI | undefined;
...
const DeployedContractProvider = ({
  ...
  const [zkbadgeApi, setZKBadgeApi] = useState<
    DeployedCertVerificationAPI | undefined
  >(undefined);
  ...
  const onJoinContract = useCallback(async () => {
    ...
    try {
      logger?.info(`GETTING CONTRACT INFO ${contractAddress}`);

      const deployedAPI =
        await CertVerificationAPI.joinCertVerificationContract(
          walletContext.walletState.providers,
          contractAddress,
          logger as any
        );

      setZKBadgeApi(deployedAPI);
      setHasJoined(true);
      ...
      setError(errMsg);
      logger?.error(
        `Failed to join contract error ${errMsg}:::: contractAddress::::${contractAddress}`
      );
    } finally {
      setIsJoining(false);
    }
...
export const useDeployedContract = (): DeploymentProvider => {
  ...
  if (!context) {
    throw new Error(
      "useDeployedContract must be used within a DeployedContractProvider"
    );
  }
