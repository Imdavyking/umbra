...
import type { CertVerificationContractProviders } from "@zkbadge/zkbadge-api";
...
export interface MidnightWalletState {
  ...
  readonly providers: CertVerificationContractProviders | undefined;
...
const MidnightWalletProvider = ({
  ...
  const connectToWalletAndInitializeProviders = useCallback(async () => {
    ...
    try {
      ...
      logger?.info(`Wallet connected successfully ${walletStateData.address}`);
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Failed to initialize wallet";
      dispatch({ type: "CONNECT_ERROR", payload: errorMessage });
      logger?.error(`Wallet connection failed ${errorMessage}`);
    }
